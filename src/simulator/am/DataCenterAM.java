...
public class DataCenterAM extends GeneralAM {
	...
	public void monitor() {
		...
		for (int i = 0; i < computeSystems.size(); i++) {
			computeSystemsSLAViolations[i] = ((GeneralAM) computeSystems.get(i).getAM()).getSLAViolationGen();
		}
	}
	...
	private GeneralAM proxyAM(ComputeSystem computeSystem) {
		return (GeneralAM) computeSystem.getAM();
	}
	...
	private void analysisGreen() {
		...
		for (int i = 0; i < computeSystemsSLAViolations.length; i++) {
			if (computeSystemsSLAViolations[i] > 0
					&& proxyAM(computeSystems.get(i)).getStrategy() == Simulator.StrategyEnum.Green) {
				proxyAM(computeSystems.get(i)).setStrategy(Simulator.StrategyEnum.SLA);
				LOGGER.info("AM in DC Switch HPC system: " + i + " to SLA  @  " + environment().getCurrentLocalTime());
			}
			if (computeSystemsSLAViolations[i] == 0
					&& proxyAM(computeSystems.get(i)).getStrategy() == Simulator.StrategyEnum.SLA) {
				LOGGER.info(
						"AM in DC Switch HPC system: " + i + "  to Green @  " + environment().getCurrentLocalTime());
				proxyAM(computeSystems.get(i)).setStrategy(Simulator.StrategyEnum.Green);
			}
		}
		/*
		 * if Slowdown from cooler begin block workload with lowest priority
		 * start a timer: â€œblock timerâ€� end if available nodes in system
		...
		if (!computeSystems.get(0).isBlocked()) {
			...
			proxyAM(computeSystems.get(1)).setStrategy(Simulator.StrategyEnum.Green);
		} else {
