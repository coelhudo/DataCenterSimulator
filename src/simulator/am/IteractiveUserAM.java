...
public class IteractiveUserAM extends GeneralAM {
    ...
    public void analysis_SLA(Object violation) {
        ...
        if (violationInEpoch > 0) {
            for (int j = 0; j < User.getComputeNodeList().size(); j++) {
                if (User.getComputeNodeList().get(j).getReady() != -1) //except idle nodes
                {
    ...
    public void analysis_GR(Object violation) {
        ...
        if (violationInEpoch == 0) {
            ...
            for (int j = 0; j < User.getComputeNodeList().size() & User.numberofRunningNode() > (User.getMinProc() + 1); j++) {
                if (User.getComputeNodeList().get(j).getReady() == 1 && User.getComputeNodeList().get(j).getCurrentCPU() == 0) {
